...
- recorded the 2026-09-21 public-SDK Port Forward read-back normalization: ACIY.3 accepted an uppercase synthetic MAC and returned the same address bytes with lowercase hexadecimal letters, so Port Forward verification now treats MAC case as representational while preserving raw getter values
